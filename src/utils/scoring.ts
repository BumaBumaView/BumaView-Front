...
export const calculateScores = (blendshapesData) => {
  console.log('[DEBUG] Calculating scores with blendshapesData (length):', blendshapesData.length);
  if (!blendshapesData || blendshapesData.length === 0) {
    console.log('[DEBUG] No blendshapes data. Returning default scores.');
    return {
  ...
  console.log('[DEBUG] Calculated scores:', result);
  return result;
};